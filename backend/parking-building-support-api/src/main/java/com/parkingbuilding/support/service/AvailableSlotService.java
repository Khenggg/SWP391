package com.parkingbuilding.support.service;

import java.util.List;

import org.springframework.stereotype.Service;

import com.parkingbuilding.support.dto.response.AvailableSlotResponse;
import com.parkingbuilding.support.sharedreadmodel.entity.AreaReadEntity;
import com.parkingbuilding.support.sharedreadmodel.entity.SlotReadEntity;
import com.parkingbuilding.support.sharedreadmodel.repository.AreaReadRepository;
import com.parkingbuilding.support.sharedreadmodel.repository.SlotReadRepository;

import lombok.RequiredArgsConstructor;

@Service
@RequiredArgsConstructor
public class AvailableSlotService {

    private final SlotReadRepository slotReadRepository;
    private final AreaReadRepository areaReadRepository;

    public List<AvailableSlotResponse> getAvailableSlots(Long vehicleTypeId,
            Long areaId,
            Long floorId) {
        System.out.println("service ok");

        List<SlotReadEntity> slots = slotReadRepository.findByStatus("AVAILABLE");

        if (vehicleTypeId != null) {
            slots = slots.stream()
                    .filter(slot -> slot.getAllowedVehicleTypeId().equals(vehicleTypeId)).toList();
        }

        if (areaId != null) {
            slots = slots.stream()
                    .filter(slot -> slot.getAreaId().equals(areaId)).toList();
        }

        if (floorId != null) {

            List<Long> areaIds = areaReadRepository.findByFloorId(floorId)
                    .stream()
                    .map(AreaReadEntity::getId)
                    .toList();

            slots = slots.stream()
                    .filter(slot -> areaIds.contains(slot.getAreaId()))
                    .toList();
        }

        return slots.stream().map(slot -> new AvailableSlotResponse(
                slot.getId(),
                slot.getSlotCode(),
                slot.getAreaId(),
                slot.getAllowedVehicleTypeId()
        )).toList();
    }

}
