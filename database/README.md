...
| Trao đổi | `notifications`, `feedbacks` | Thông báo theo nghiệp vụ và phản hồi của người dùng |
| Xử lý sự cố | `lost_card_cases`, `lost_card_case_documents`, `lost_card_refunds`, `plate_mismatch_cases`, `audit_logs` | Mất thẻ, hồ sơ minh chứng, hoàn phí, lệch biển số, audit |
...
| `deleted_at` | Thời điểm soft-delete; nullable khi tài khoản còn hoạt động. |

### `auth_sessions`, `refresh_tokens`, `revoked_access_tokens`

Các bảng xác thực do backend sở hữu và đã bật Row Level Security để không bị truy cập trực tiếp qua Supabase Data API.

| Bảng | Ý nghĩa |
| --- | --- |
| `auth_sessions` | Một họ phiên đăng nhập, gồm thời hạn và thông tin thu hồi. |
| `refresh_tokens` | Refresh token đã hash, thuộc một `auth_sessions`; hỗ trợ rotation và thu hồi cả họ token. |
| `revoked_access_tokens` | JWT access token đã bị thu hồi trước hạn để backend chặn ngay. |
...
| `brand`, `color` | Thông tin mô tả xe, nullable. |
| `approval_status` | Trạng thái duyệt xe: `PENDING`, `APPROVED`, `REJECTED`. |
...
| `floor_id`, `area_id`, `slot_id` | Vị trí được cấp cho vé; nullable tùy mô hình quản lý theo tầng/khu/ô. |
| `start_date`, `end_date` | Khoảng hiệu lực. |
| `status` | `ACTIVE`, `EXPIRED`, `LOCKED`. |
| `created_by` | Nhân sự tạo vé. |

### `monthly_pass_applications`

Đơn đăng ký vé tháng trước khi trở thành một `monthly_passes` hoạt động.

| Field | Ý nghĩa |
| --- | --- |
| `driver_id`, `vehicle_id` | Người đăng ký và xe đã đăng ký được dùng cho đơn. |
| `start_date`, `price` | Ngày muốn bắt đầu và giá được duyệt; giá không âm. |
| `cccd_front_image_url`, `cccd_back_image_url`, `face_image_url`, `plate_image_url` | Hồ sơ ảnh phục vụ duyệt đơn. |
| `status` | `DRAFT`, `PENDING`, `APPROVED_AWAITING_PAYMENT`, `PAID`, `ACTIVE`, `EXPIRED`, `REJECTED`. |
| `payment_method`, `payment_reference_no` | Thông tin thanh toán; bắt buộc có phương thức khi đơn đã `PAID` hoặc `ACTIVE`. |
| `assigned_card_id` | Thẻ được cấp; bắt buộc khi đơn `ACTIVE`. |
| `rejection_reason`, `note` | Lý do từ chối và ghi chú xử lý. |
...
| `plate_number`, `normalized_plate_number` | Biển số snapshot của lượt đặt. Có thể cùng null khi reservation dựa vào `vehicle_id`; nếu có thì cả hai phải khác rỗng. |
...
| `payment_deadline`, `confirmed_at` | Hạn thanh toán và thời điểm reservation được xác nhận. |
...
### `lost_card_case_documents`

Tài liệu/ảnh minh chứng của một case mất thẻ. File được soft-delete bằng `deleted_at`; mỗi loại tài liệu khác `OTHER` chỉ có một bản active trong một case.

| Field | Ý nghĩa |
| --- | --- |
| `lost_card_case_id` | Case sở hữu tài liệu; xóa case sẽ xóa tài liệu liên quan. |
| `document_type`, `file_path`, `thumbnail_path` | Loại và đường dẫn file/thumbnail. |
| `original_file_name`, `mime_type`, `size_bytes`, `sha256_hash` | Metadata dùng để kiểm tra và truy vết file. |
| `is_sensitive`, `uploaded_by`, `uploaded_at`, `deleted_at` | Phân loại nhạy cảm và audit vòng đời tài liệu. |
...
Thông báo gửi cho một user, có thể gắn với vé tháng, reservation, payment hoặc parking session. Khi `is_read = true`, `read_at` bắt buộc phải có; khi chưa đọc, `read_at` phải null.

### `feedbacks`

Phản hồi của người dùng hoặc khách chưa đăng nhập. `user_id`, `parking_session_id`, `reservation_id` đều nullable; khi nhân viên phản hồi thì `responded_by` và `responded_at` phải được ghi cùng nhau.
...
- Reservation `CONFIRMED/PAID` phải có snapshot pricing, `confirmed_at` và payment đã thanh toán tương ứng.
- `notifications` giữ audit đọc bằng cặp `is_read/read_at`; `feedbacks` giữ audit phản hồi bằng cặp `responded_by/responded_at`.
...
Ba script có transaction và có thể chạy lại theo đúng thứ tự trên cùng database. Lần chạy lại dùng `IF NOT EXISTS`/`ON CONFLICT` và tái tính trạng thái slot/khu từ reservation seed thay vì phụ thuộc vào giá trị cũ.
...
- Không thêm `ALTER TABLE` lúc backend khởi động; thay đổi cấu trúc phải nằm trong script database có thể kiểm thử độc lập.
