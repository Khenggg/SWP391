...
export default function ExitPayment({ session, fee, canExit, isZeroCharge, hasPendingOnlinePayment, payosPaymentUrl, onShowQrModal, isLoading, handleRequestCash, handlePayOS, handleCompleteExitPaid, handleCompleteMonthlyExit, refreshSession, mismatchBlocked, mismatchStatus, hasExitImages }) {
  ...
  const needsPayment = !isMonthly && (!isPaid || (isPaid && isBufferExpired && !isZeroCharge));
  ...
  return (
    <section className="mt-auto flex shrink-0 flex-col gap-4 rounded-xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
      <div className="mb-2 flex items-center gap-2">
        <span className="flex size-5 items-center justify-center rounded-full bg-indigo-600 text-[10px] font-bold text-white">6</span>
        <h3 className="text-sm font-bold text-slate-800">Thanh toán và xác nhận</h3>
      </div>
      {!session ? (
        <div className="flex items-center justify-center py-6 text-sm font-medium text-slate-400">Nhập thẻ hoặc biển số để tiếp tục</div>
      ) : mismatchBlocked ? (
        <div className="flex flex-col items-center justify-center gap-2 py-6 text-center">
          <Lock className="size-8 text-slate-400" />
          <p className="text-sm font-bold text-slate-600">Thanh toán và ra xe bị tạm khóa</p>
          <p className="text-xs text-slate-500">{blockedMessage}</p>
          <Button variant="ghost" onClick={refreshSession} disabled={isLoading} className="mt-1 h-8 text-xs text-slate-500 hover:text-slate-800">
            <RefreshCw className={`mr-1 size-3.5 ${isLoading ? "animate-spin" : ""}`} />Làm mới trạng thái
          </Button>
        </div>
      ) : (
        <>
          {!hasExitImages && (
            <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3">
              ...
              <div>
                <p className="text-xs font-bold text-amber-900">Thiếu ảnh xe ra</p>
                <p className="mt-0.5 text-[10px] text-amber-700">Tải cả ảnh biển số và ảnh toàn xe trước khi xác nhận xe ra.</p>
              </div>
            </div>
          )}
          ...
          {!isMonthly && isZeroCharge && (
            <Button className="h-14 w-full bg-emerald-600 text-lg font-black text-white shadow-lg shadow-emerald-200 hover:bg-emerald-700" onClick={handleCompleteExitPaid} disabled={!canExit || isLoading}>
          ...
          {needsPayment && (
            <div className="flex flex-col gap-3">
              ...
              <Button variant="ghost" onClick={refreshSession} disabled={isLoading} className="h-8 self-end text-xs text-slate-500 hover:text-slate-800">
                <RefreshCw className={`mr-1 size-3.5 ${isLoading ? "animate-spin" : ""}`} />Làm mới trạng thái thanh toán
              </Button>
            </div>
          )}

          {!isMonthly && isPaid && !isBufferExpired && (
            <Button className="h-14 w-full bg-emerald-600 text-lg font-black text-white shadow-lg shadow-emerald-200 hover:bg-emerald-700" onClick={handleCompleteExitPaid} disabled={!canExit || isLoading}>
              Xác nhận xe ra (Đã thanh toán)
              <ArrowRight className="ml-2 size-5" />
            </Button>
          )}

          {isMonthly && (
            <Button className="h-14 w-full bg-emerald-600 text-lg font-black text-white shadow-lg shadow-emerald-200 hover:bg-emerald-700" onClick={handleCompleteMonthlyExit} disabled={!canExit || isLoading}>
