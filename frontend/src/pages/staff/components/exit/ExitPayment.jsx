import { ArrowRight, Camera, Check, Lock, QrCode, RefreshCw } from "lucide-react";
import { Button } from "@/components/ui/button";

export default function ExitPayment({ session, fee, canExit, isZeroCharge, hasPendingOnlinePayment, isLoading, handleRequestCash, handlePayOS, handleCompleteExitPaid, handleCompleteMonthlyExit, refreshSession, mismatchBlocked, mismatchStatus, hasExitImages, cancelQR }) {
  ...
  return (
    <section className="mt-auto flex shrink-0 flex-col gap-4 rounded-xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
      ...
      {!session ? <div className="flex items-center justify-center py-6 text-sm font-medium text-slate-400">Nhập thẻ hoặc biển số để tiếp tục</div> : mismatchBlocked ? (
        ...
        {!isMonthly && !isPaid && !isZeroCharge && <div className="flex flex-col gap-3">{isWaitingOnline ? <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-center"><p className="text-sm font-bold text-amber-800">Đang chờ PayOS xác minh thanh toán</p><p className="mt-1 mb-3 text-xs text-amber-700">Xe chỉ được ra sau khi webhook cập nhật trạng thái PAID.</p><Button variant="outline" size="sm" onClick={cancelQR} disabled={isLoading} className="border-amber-400 text-amber-700 hover:bg-amber-100 font-bold">Hủy QR / Chọn lại phương thức</Button></div> : <div className="grid grid-cols-2 gap-3"><Button variant="outline" onClick={handleRequestCash} disabled={isLoading || !fee} className="h-12 border-2 border-indigo-600 bg-indigo-50 font-bold text-indigo-700 hover:bg-indigo-100"><Check className="mr-2 size-5" />Tiền mặt</Button><Button variant="outline" onClick={handlePayOS} disabled={isLoading || !fee} className="h-12 border-2 border-emerald-600 bg-emerald-50 font-bold text-emerald-700 hover:bg-emerald-100"><QrCode className="mr-2 size-5" />Chuyển khoản (QR)</Button></div>}<Button variant="ghost" onClick={refreshSession} disabled={isLoading} className="h-8 self-end text-xs text-slate-500 hover:text-slate-800"><RefreshCw className={`mr-1 size-3.5 ${isLoading ? "animate-spin" : ""}`} />Làm mới trạng thái thanh toán</Button></div>}
