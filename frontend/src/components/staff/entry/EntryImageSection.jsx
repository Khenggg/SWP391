  ...
  return (
    <div className="flex min-w-0 flex-col gap-2">
      ...
      <div className="flex gap-2">
        <Button type="button" variant="outline" size="sm" disabled={isDisabled} className="h-8 flex-1 text-xs font-bold" onClick={() => inputRef.current?.click()}>
          <Upload className="mr-1.5 size-3.5" />
          {isUploading ? "Đang lưu..." : value ? "Đổi ảnh" : "Tải ảnh"}
        </Button>
        {value && (
          <Button type="button" variant="outline" size="icon" disabled={isDisabled} aria-label={`Xóa ${label}`} className="size-8 border-red-200 text-red-600 hover:bg-red-50 hover:text-red-700" onClick={() => void onChange("")}>
