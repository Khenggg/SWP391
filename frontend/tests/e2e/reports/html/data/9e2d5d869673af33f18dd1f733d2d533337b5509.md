...
- Name: p1\public-pages.spec.ts >> Public Pages Acceptance Suite >> Available Slots >> 4.6 Empty State - displays message when no slots match
- Location: tests\e2e\specs\p1\public-pages.spec.ts:213:5
...
Locator: getByText(/no available slots|không có|trống/i)
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for getByText(/no available slots|không có|trống/i)

```

```yaml
- img
- heading "404 - Không Tìm Thấy Trang" [level=1]
- paragraph: Đường dẫn bạn truy cập hiện tại không tồn tại hoặc đã được chuyển sang khu vực khác.
